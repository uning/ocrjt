...
const helpStr = `
会在output目录下,将截图目录扁平化建立目录,将识别结果写入该目录下ocr.xlst,ocr.csv
出错的文件写入 ocr.csv.err
...
async function processAll() {
  ...
  if (path.isAbsolute(sourceDirectory))
    sourceDirectory = path.path.resolve(imgdir);
  ...
  const csvhead = '文件,日期,下单人,手机号,地址,实付款,订单号,下单时间,商品总价,商品名,其他信息' + "\n";
  ...
  */
  const imageFiles  = [];
  readImageFiles(sourceDirectory,imageFiles);
  console.log('总共 ' +imageFiles.length+' 个文件');
  // return;
  // 遍历每个文件
  let i = 0;

  const wsData = [];
  wsData.push(['文件(可点击打开)', '日期', '下单人', '手机号', '地址', '实付款', '订单号', '下单时间', '商品总价', '商品名', '其他信息']);
  ...
  const wsName = imgdir.replace(/\./g, '').replace(/\//g, '_');
